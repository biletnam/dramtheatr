...
$res = mysql_query("SELECT * FROM dt_userlist WHERE login='".$_SESSION['login']."' AND pass='".$_SESSION['pass']."'", $db);
        ...
        <fieldset>
          ...
          <form action="teatradmin/sub.php" method="POST" enctype=multipart/form-data>
            ...
            <select name="smena_tema" id="smena_tema">
              ...
              $myrow = mysql_fetch_array($result);
              do {
                printf ("<p><option value='%s'>%s</option></p>", $myrow["id"], $myrow["nazva"]);
              } while ($myrow=mysql_fetch_array($result));
              ?>
          ...
          if ($handle = opendir("img/")) {
            while ($entry = readdir($handle)) {
              ...
              if ($entry == $myrow["photo"]) {
                ...
                do {
                  printf ("<input name='checkbox' id='%s' class='my-checkbox'
                    type='checkbox' value='%s'></input>
